...
if __name__ == '__main__':
    ...
    if args.download:
        ...
        local_filename, df = sw.export_judge_list()
        ...
        print("\n origin fair:")
        c = df["ORIGIN FAIR"].value_counts(dropna=False)
        p = df["ORIGIN FAIR"].value_counts(dropna=False, normalize=True).mul(100).round(1)
        print()
        print(pd.concat([c, p], axis=1, keys=['students', '%']))
        for column in ['approval', 'payment', 'final']:
            print(f'\n{column} status:')
            c = df[f"{column.upper()} STATUS"].value_counts(dropna=False)
            p = df[f"{column.upper()} STATUS"].value_counts(dropna=False, normalize=True).mul(100).round(1)
